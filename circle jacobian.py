...
class ScaraRobot:
    ...
    def calculate_jacobian_parallel(self, theta1_deg, theta2_deg):
        """Tính ma trận Jacobian cho robot song song 5 thanh"""
        theta1 = np.radians(theta1_deg)
        theta2 = np.radians(theta2_deg)

        # Lấy thông số từ robot_params
        L1 = self.robot_params['L1']  # 15.0 cm
        L2 = self.robot_params['L2']  # 25.0 cm
        x5 = self.robot_params['x5']  # -20.0 cm (động cơ bên phải)
        ...
        # Tính vị trí hiện tại của đầu bút (điểm x3)
        fk_result = self.forward_kinematics(theta1_deg, theta2_deg)
        if not fk_result or fk_result[1] is None:
            # Trả về ma trận đơn vị nếu FK thất bại
            return np.eye(2)
        x3, y3 = fk_result[1]

        # Tính các vector đơn vị
        v1 = np.array([x3 - x2, y3 - y2])  # Vector từ x2 đến x3
        v2 = np.array([x3 - x4, y3 - y4])  # Vector từ x4 đến x3
        ...
        if v1_norm < 1e-6 or v2_norm < 1e-6:
            return np.eye(2)  # Trả về ma trận đơn vị nếu có vấn đề

        v1 = v1 / v1_norm
        v2 = v2 / v2_norm

        # Tính ma trận Jacobian cho robot song song
        # Ma trận này liên hệ tốc độ góc của hai motor với tốc độ Cartesian của điểm x3

        # J_x = [J_11 J_12; J_21 J_22]
        # Với J_x * [dx; dy] = [dtheta1; dtheta2]

        # Tính J_p (Jacobian vị trí)
        J_p = np.zeros((2, 2))

        # Liên hệ tốc độ góc với tốc độ Cartesian qua L2 (từ motor 1)
        J_p[0, 0] = -L1 * np.sin(theta1)  # dx/dtheta1
        J_p[0, 1] = L1 * np.cos(theta1)  # dy/dtheta1

        # Liên hệ tốc độ góc với tốc độ Cartesian qua L4 (từ motor 2)
        J_p[1, 0] = -L4 * np.sin(theta2)  # dx/dtheta2
        J_p[1, 1] = L4 * np.cos(theta2)  # dy/dtheta2

        # Tính ma trận ràng buộc do cấu trúc song song
        C = np.array([v1, v2])

        # Kết hợp để có Jacobian cuối cùng
        J_x = np.linalg.inv(C) @ J_p

        # Đảo ngược để chuyển từ dtheta/dxy sang dxy/dtheta
        J = np.linalg.inv(J_x)

        return J

    def generate_circle_path(self, center_x, center_y, radius, steps=800):
        """
        Tạo đường tròn dựa trên tâm và bán kính

        Tham số:
            center_x, center_y: tọa độ tâm đường tròn
            radius: bán kính đường tròn
            steps: số điểm trên đường tròn (mặc định 36 điểm, tương ứng 10° mỗi bước)

        Trả về:
            Danh sách các tọa độ điểm (x,y) trên đường tròn
        """
        import numpy as np
        import logging

        # Chuyển đổi tọa độ hiển thị sang tọa độ nội bộ của robot
        center_x_internal = -center_x

        # Kiểm tra xem toàn bộ đường tròn có nằm trong không gian làm việc không
        points_in_workspace = True
        for angle in np.linspace(0, 2 * np.pi, 8):  # Kiểm tra 8 điểm xung quanh đường tròn
            x = center_x_internal + radius * np.cos(angle)
            y = center_y + radius * np.sin(angle)
            if not self.check_point_in_workspace(x, y):
                points_in_workspace = False
                break

        if not points_in_workspace:
            logging.warning(f"Đường tròn vượt quá không gian làm việc: tâm=({center_x},{center_y}), R={radius}")
            return []

        # Tạo danh sách các điểm trên đường tròn
        circle_points = []
        for i in range(steps + 1):  # +1 để khép kín đường tròn
            angle = 2 * np.pi * i / steps
            x_internal = center_x_internal + radius * np.cos(angle)
            y = center_y + radius * np.sin(angle)

            # Chuyển lại sang tọa độ hiển thị
            x_display = -x_internal
            circle_points.append((x_display, y))

        return circle_points

    def move_in_straight_line_jacobian_parallel(self, start_x, start_y, end_x, end_y, max_step_size=0.02, steps=None):
        """Di chuyển theo đường thẳng sử dụng phương pháp Jacobian cho robot song song"""
        import logging
        gcode_lines = []
        ...
        try:
            ...
            logging.error("Tọa độ đầu vào không hợp lệ")
            return gcode_lines

        # Chuyển đổi tọa độ hiển thị sang tọa độ nội bộ
        start_x_internal = -start_x
        end_x_internal = -end_x

        # Kiểm tra không gian làm việc
        if not (self.check_point_in_workspace(start_x_internal, start_y) and
                self.check_point_in_workspace(end_x_internal, end_y)):
            logging.warning(f"Điểm nằm ngoài không gian làm việc")
            return gcode_lines

        # Tính IK cho điểm bắt đầu
        initial_ik = self.inverse_kinematics(start_x_internal, start_y)
        if initial_ik is None or initial_ik[0] is None or initial_ik[1] is None:
            logging.error(f"Lỗi IK cho điểm bắt đầu")
            return gcode_lines

        theta1, theta2, config = initial_ik

        # Thêm lệnh di chuyển đến điểm bắt đầu
        gcode_lines.append(f"G1 X{start_x:.3f} Y{start_y:.3f}")

        # Tính tổng khoảng cách di chuyển
        dx_total = end_x_internal - start_x_internal
        dy_total = end_y - start_y
        distance = np.sqrt(dx_total ** 2 + dy_total ** 2)

        # Xác định số bước
        if steps is None:
            steps = max(10, int(distance / max_step_size))

        # Di chuyển theo từng bước nhỏ
        x, y = start_x_internal, start_y
        for i in range(1, steps + 1):
            try:
                # Tính ma trận Jacobian cho robot song song
                J = self.calculate_jacobian_parallel(theta1, theta2)

                # Tính bước di chuyển hiện tại
                t = i / steps
                x_target = start_x_internal + dx_total * t
                y_target = start_y + dy_total * t

                # Tính sai số hiện tại
                dx = x_target - x
                dy = y_target - y

                # Tính gia số góc thông qua Jacobian
                # [dtheta1, dtheta2] = J^-1 * [dx, dy]
                delta = np.dot(np.linalg.inv(J), np.array([dx, dy]))
                delta_theta1, delta_theta2 = delta
                ...
                theta1_min, theta1_max = self.angle_limits['theta1_min'], self.angle_limits['theta1_max']
                theta2_min, theta2_max = self.angle_limits['theta2_min'], self.angle_limits['theta2_max']

                if not (theta1_min <= theta1 <= theta1_max and theta2_min <= theta2 <= theta2_max):
                    logging.warning(f"Vượt quá giới hạn góc")
                    theta1 = min(max(theta1, theta1_min), theta1_max)
                    theta2 = min(max(theta2, theta2_min), theta2_max)

                # Tính vị trí mới từ góc mới (FK)
                fk_result = self.forward_kinematics(theta1, theta2)
                if fk_result and fk_result[1] is not None:
                    x, y = fk_result[1]
                else:
                    logging.error(f"Lỗi FK tại bước {i}")
                    continue
                ...
                # Thêm lệnh G-code
                gcode_lines.append(f"G1 X{x_display:.3f} Y{y:.3f}")

            except Exception as e:
                logging.error(f"Lỗi trong phương pháp Jacobian: {e}")
                continue

        return gcode_lines

    def log(self, message):
        """Hàm log đơn giản để phương thức Jacobian có thể ghi log"""
        import logging
    ...
    def move_in_straight_line(self, start_x, start_y, end_x, end_y, steps=20, use_jacobian=True):
        """Di chuyển robot theo đường thẳng - có thể chọn phương pháp nội suy hoặc Jacobian"""
        if use_jacobian:
            return self.move_in_straight_line_jacobian(start_x, start_y, end_x, end_y, steps=steps)
        else:
            gcode_lines = []

            # Đảm bảo điểm đầu và cuối nằm trong không gian làm việc
            if not (self.check_point_in_workspace(-start_x, start_y) and
                    self.check_point_in_workspace(-end_x, end_y)):
                print(f"Điểm nằm ngoài không gian làm việc: ({start_x},{start_y}) -> ({end_x},{end_y})")
                return gcode_lines

            # Tạo các điểm trung gian trên đường thẳng
            for i in range(steps + 1):
                t = i / steps
                x = start_x + (end_x - start_x) * t
                y = start_y + (end_y - start_y) * t

                # Thêm lệnh di chuyển đến điểm này
...
class ScaraGUI:
    ...
    def draw_circle(self, center_x, center_y, radius):
        """Vẽ đường tròn sử dụng phương pháp Jacobian"""
        if not self.is_connected:
            self.log("Chưa kết nối! Không thể vẽ đường tròn.", tag="ERROR")
            return False

        # Tạo đường tròn
        circle_points = self.robot.generate_circle_path(center_x, center_y, radius)
        if not circle_points:
            self.log(f"Không thể tạo đường tròn với tâm ({center_x}, {center_y}), R={radius}", tag="ERROR")
            return False

        self.log(f"Vẽ đường tròn: tâm=({center_x}, {center_y}), R={radius}, {len(circle_points)} điểm")

        # Di chuyển đến điểm đầu tiên (pen up)
        first_point = circle_points[0]
        x_start, y_start = first_point

        # Tính góc cho điểm đầu tiên
        x_internal = -x_start
        ik_res = self.robot.inverse_kinematics(x_internal, y_start, self.current_config)
        if ik_res is None:
            self.log(f"Lỗi IK cho điểm bắt đầu: ({x_start}, {y_start})", tag="ERROR")
            return False

        t1, t2, cfg = ik_res
        cmd = f"{t1:.2f},{t2:.2f}"

        # Nâng bút và di chuyển đến điểm đầu
        self.pen_up()
        if not self.send_command(cmd, wait_for_response=True):
            self.log("Lỗi di chuyển đến điểm bắt đầu đường tròn", tag="ERROR")
            return False

        if not self.wait_for_command_completion():
            self.log("Timeout chờ di chuyển đến điểm bắt đầu", tag="ERROR")
            return False

        # Hạ bút
        self.pen_down()

        # Vẽ đường tròn từng đoạn thẳng ngắn
        success = True
        for i in range(1, len(circle_points)):
            ...
            prev_x, prev_y = circle_points[i - 1]
            next_x, next_y = circle_points[i]

            # Sử dụng phương pháp Jacobian hoặc nội suy thông thường
            use_jacobian = hasattr(self, 'use_jacobian_var') and self.use_jacobian_var.get()
            if use_jacobian and hasattr(self.robot, 'move_in_straight_line_jacobian_parallel'):
                # Tính khoảng cách đoạn thẳng
                dist = ((next_x - prev_x) ** 2 + (next_y - prev_y) ** 2) ** 0.5
                step_size = 0.05  # cm

                gcode_lines = self.robot.move_in_straight_line_jacobian_parallel(
                    prev_x, prev_y, next_x, next_y, max_step_size=step_size)

                for cmd in gcode_lines:
                    if cmd.startswith("G1 X"):
                        parts = cmd.split()
                        x_val = float(parts[1][1:])
                        y_val = float(parts[2][1:])

                        # Tính góc
                        x_internal = -x_val
                        ik_res = self.robot.inverse_kinematics(x_internal, y_val, self.current_config)
                        if ik_res is None:
                            continue

                        t1, t2, cfg = ik_res
                        angle_cmd = f"{t1:.2f},{t2:.2f}"

                        if not self.send_command(angle_cmd, wait_for_response=True):
                            success = False
                            break
                ...
                # Phương pháp nội suy thông thường
                if not self._interpolate_and_move(prev_x, prev_y, next_x, next_y, step_size_cm=0.05):
                    success = False
                    break

        # Nâng bút khi hoàn thành
        self.pen_up()

        if success:
            self.log("Đã hoàn thành vẽ đường tròn", tag="SUCCESS")
        else:
            self.log("Vẽ đường tròn bị gián đoạn", tag="WARNING")

        return success

    def _interpolate_and_move(self, x_start_disp, y_start, x_end_disp, y_end, step_size_cm=0.01):
        """Nội suy đường thẳng trong không gian Cartesian và gửi lệnh góc cho từng điểm."""
        # Kiểm tra đầu vào
        if x_start_disp is None or y_start is None or x_end_disp is None or y_end is None:
            self.log(f"(!) Lỗi: Tọa độ đầu vào có giá trị None", tag="ERROR")
            return False

        # Sử dụng phương pháp Jacobian nếu có
        use_jacobian = False
        if hasattr(self, 'use_jacobian_var') and self.use_jacobian_var is not None:
            try:
                use_jacobian = self.use_jacobian_var.get()
            except Exception:
                use_jacobian = False

        if use_jacobian and hasattr(self.robot, 'move_in_straight_line_jacobian'):
            try:
                # Sử dụng phương pháp Jacobian
                gcode_lines = self.robot.move_in_straight_line_jacobian(
                    x_start_disp, y_start, x_end_disp, y_end, max_step_size=step_size_cm)

                if not gcode_lines:
                    self.log(f"(!) Không thể tạo G-code với Jacobian", tag="ERROR")
                    return False
                ...
                for cmd in gcode_lines:
                    ...
                    if cmd.startswith("G1 X"):
                        try:
                            ...
                            x_internal = -x_val
                            ik_res = self.robot.inverse_kinematics(x_internal, y_val, self.current_config)
                            if ik_res is None or ik_res[0] is None or ik_res[1] is None:
                                self.log(f"(!) Lỗi IK cho điểm Jacobian ({x_val:.2f}, {y_val:.2f})", tag="ERROR")
                                success = False
                                break
                            ...
                            # Gửi lệnh góc và chờ hoàn thành
                            if not self.send_command(angle_cmd, wait_for_response=True):
                                self.log(f"(!) Lỗi gửi lệnh Jacobian", tag="ERROR")
                                success = False
                                break

                            # Đợi lệnh hoàn thành xử lý trong hàng đợi
                            if not self.wait_for_command_completion():
                                self.log(f"(!) Timeout chờ lệnh Jacobian", tag="ERROR")
                                success = False
                                break
                        except Exception as e:
                            self.log(f"(!) Lỗi xử lý lệnh Jacobian: {e}", tag="ERROR")
                            success = False
                            break

                return success
            except Exception as e:
                self.log(f"(!) Lỗi sử dụng Jacobian: {e}", tag="ERROR")
                # Tiếp tục với phương pháp thông thường nếu Jacobian thất bại

        # Phương pháp nội suy thông thường (giữ code hiện tại)
        x_start_int = -x_start_disp
        x_end_int = -x_end_disp

        dx = x_end_int - x_start_int
        dy = y_end - y_start
        distance = math.sqrt(dx * dx + dy * dy)

        if distance < 1e-3:  # Quá gần, không cần nội suy
            return True

            # ... (tiếp tục code hiện tại của bạn)

            # Số bước nội suy, đảm bảo ít nhất 1 bước
        num_steps = max(1, int(distance / step_size_cm))
        logging.debug(
            f"Nội suy thẳng: ({x_start_disp:.2f},{y_start:.2f}) -> ({x_end_disp:.2f},{y_end:.2f}), Dist: {distance:.2f}cm, Steps: {num_steps}")

        success = True
        for i in range(1, num_steps + 1):  # Bắt đầu từ 1 vì điểm 0 là vị trí hiện tại
            t = i / num_steps
            xi_int = x_start_int + dx * t
            yi = y_start + dy * t

            # Kiểm tra điểm trung gian
            if not self.robot.check_point_in_workspace(xi_int, yi):
                self.log(f"(!) Điểm nội suy ({-xi_int:.2f}, {yi:.2f}) ngoài vùng!", tag="WARNING")
                # Có thể chọn dừng hoặc bỏ qua điểm này
                continue  # Bỏ qua điểm này và tiếp tục

            # Tính IK cho điểm trung gian
            ik_res = self.robot.inverse_kinematics(xi_int, yi, self.current_config)
            if ik_res is None:
                self.log(f"(!) Lỗi IK cho điểm nội suy ({-xi_int:.2f}, {yi:.2f})", tag="ERROR")
                success = False
                break  # Dừng nếu IK lỗi

            t1, t2, cfg = ik_res
            cmd = f"{t1:.2f},{t2:.2f}"

            # Gửi lệnh góc và chờ hoàn thành *cho từng điểm*
            if not self.send_command(cmd, wait_for_response=True):
                self.log(f"(!) Lỗi gửi lệnh nội suy {i}/{num_steps}", tag="ERROR")
                success = False
                break
            # Đợi lệnh hoàn thành xử lý trong hàng đợi
            if not self.wait_for_command_completion():
                self.log(f"(!) Timeout chờ lệnh nội suy {i}/{num_steps}", tag="ERROR")
                success = False
                break

            # Kiểm tra cờ dừng G-code
            if self.stop_gcode_flag:
                success = False
                break

            # Cập nhật vị trí hiện tại (tạm thời) để bước nội suy tiếp theo chính xác hơn
            # (Arduino sẽ báo lại vị trí cuối cùng chính xác sau khi lệnh cuối hoàn thành)
            self.current_position['x'] = xi_int
            self.current_position['y'] = yi
            self.current_angles['theta1'] = t1
            self.current_angles['theta2'] = t2
            if cfg != self.current_config: self._update_ik_config(cfg)
            # Cập nhật nhanh UI (tùy chọn, có thể làm chậm nếu quá nhiều bước)
            # self.root.after(0, self.update_position_display)
            # self.root.after(0, self.update_animation)

        # Cập nhật lại vị trí cuối cùng một cách chính xác sau khi toàn bộ nội suy hoàn tất
        # (dựa trên phản hồi cuối cùng từ Arduino sẽ được xử lý bởi update_position_from_xy)
        # self.update_position_from_xy(x_end_int, y_end) # Có thể không cần nếu wait_for_command_completion cuối cùng đã cập nhật
    ...
    def create_widgets(self):
        ...
        self.use_jacobian_var = tk.BooleanVar(value=False)  # Mặc định tắt
        jacobian_check = ttk.Checkbutton(manual_frame, text="Sử dụng Jacobian",
                                         variable=self.use_jacobian_var)
        jacobian_check.grid(row=5, column=0, columnspan=2, sticky=tk.W, pady=5)
        # Trong func_frame hoặc manual_frame
        circle_frame = ttk.LabelFrame(control_panel, text="Vẽ đường tròn", padding=10)
        circle_frame.grid(row=6, column=0, sticky=tk.EW, pady=(0, 10))
        circle_frame.columnconfigure(1, weight=1)
        circle_frame.columnconfigure(3, weight=1)

        ttk.Label(circle_frame, text="Tâm X:").grid(row=0, column=0, padx=(0, 2))
        self.circle_x_entry = ttk.Entry(circle_frame, width=7)
        self.circle_x_entry.grid(row=0, column=1, padx=(0, 5), sticky=tk.EW)
        self.circle_x_entry.insert(0, "0.0")

        ttk.Label(circle_frame, text="Y:").grid(row=0, column=2, padx=(5, 2))
        self.circle_y_entry = ttk.Entry(circle_frame, width=7)
        self.circle_y_entry.grid(row=0, column=3, padx=(0, 5), sticky=tk.EW)
        self.circle_y_entry.insert(0, "30.0")

        ttk.Label(circle_frame, text="Bán kính:").grid(row=1, column=0, padx=(0, 2))
        self.circle_r_entry = ttk.Entry(circle_frame, width=7)
        self.circle_r_entry.grid(row=1, column=1, padx=(0, 5), sticky=tk.EW)
        self.circle_r_entry.insert(0, "5.0")

        self.draw_circle_btn = ttk.Button(circle_frame, text="Vẽ đường tròn", command=self.start_drawing_circle)
        self.draw_circle_btn.grid(row=1, column=2, columnspan=2, padx=5, pady=(5, 0), sticky=tk.EW)
        ...
        # Trong hàm create_widgets của class ScaraGUI
        self.use_jacobian_var = tk.BooleanVar(value=True)
        # --- Log Frame ---
    ...
    def start_drawing_circle(self):
        """Bắt đầu vẽ đường tròn với thông số từ giao diện"""
        if not self.is_connected:
            messagebox.showwarning("Chưa kết nối", "Vui lòng kết nối với robot trước!")
            return

        try:
            center_x = float(self.circle_x_entry.get())
            center_y = float(self.circle_y_entry.get())
            radius = float(self.circle_r_entry.get())

            if radius <= 0:
                messagebox.showwarning("Lỗi", "Bán kính phải lớn hơn 0!")
                return

            # Nếu đang chạy G-code, hỏi người dùng
            if self.gcode_running:
                if not messagebox.askyesno("Xác nhận", "Đang chạy G-code. Dừng và vẽ đường tròn?"):
                    return
                self.stop_gcode()

            # Disable các nút điều khiển
            self.direct_controls_enable(False)
            self.draw_circle_btn.config(state=tk.DISABLED)

            # Vẽ đường tròn trong một thread riêng
            threading.Thread(target=self._draw_circle_thread,
                             args=(center_x, center_y, radius),
                             daemon=True).start()

        except ValueError:
            messagebox.showerror("Lỗi", "Vui lòng nhập số hợp lệ cho tâm và bán kính!")
        except Exception as e:
            messagebox.showerror("Lỗi", f"Lỗi không xác định: {e}")

    def _draw_circle_thread(self, center_x, center_y, radius):
        """Thread vẽ đường tròn với xử lý rung lắc"""
        try:
            # Lưu tốc độ hiện tại
            original_speed = None
            if hasattr(self, 'speed_scale'):
                original_speed = self.speed_scale.get()

            # Điều chỉnh tốc độ dựa trên bán kính để giảm rung
            if original_speed is not None:
                adjusted_speed = original_speed
                if radius < 3.0:
                    adjusted_speed = max(10, original_speed * 0.5)  # Giảm 50% tốc độ cho đường tròn nhỏ
                    self.root.after(0, lambda: self.speed_scale.set(adjusted_speed))
                elif radius < 7.0:
                    adjusted_speed = max(10, original_speed * 0.7)  # Giảm 30% tốc độ
                    self.root.after(0, lambda: self.speed_scale.set(adjusted_speed))

                # Đợi để đảm bảo tốc độ được cập nhật
                time.sleep(0.2)

            # Thực hiện vẽ đường tròn
            self.root.after(0, lambda: self.log(
                f"Bắt đầu vẽ đường tròn: tâm=({center_x:.2f}, {center_y:.2f}), R={radius:.2f}", tag="INFO"))
            result = self.draw_circle(center_x, center_y, radius)

            # Khôi phục tốc độ ban đầu
            if original_speed is not None:
                self.root.after(0, lambda: self.speed_scale.set(original_speed))

            # Enable lại các nút sau khi vẽ xong
            self.root.after(0, self.direct_controls_enable, True)
            self.root.after(0, lambda: self.draw_circle_btn.config(state=tk.NORMAL))

            if result:
                self.root.after(0, lambda: self.log("✓ Hoàn thành vẽ đường tròn", tag="SUCCESS"))
            else:
                self.root.after(0, lambda: self.log("❌ Vẽ đường tròn không thành công", tag="ERROR"))

        except Exception as e:
            # Xử lý lỗi và đảm bảo trả về trạng thái ban đầu
            import traceback
            error_details = traceback.format_exc()
            self.root.after(0, lambda: self.log(f"❌ Lỗi vẽ đường tròn: {e}", tag="ERROR"))
            self.root.after(0, lambda: self.log(f"Chi tiết lỗi: {error_details}", tag="DEBUG"))

            # Khôi phục tốc độ nếu đã thay đổi
            if original_speed is not None:
                self.root.after(0, lambda: self.speed_scale.set(original_speed))

            # Enable lại các điều khiển
            self.root.after(0, self.direct_controls_enable, True)
            self.root.after(0, lambda: self.draw_circle_btn.config(state=tk.NORMAL))

            # Nâng bút trong trường hợp lỗi để tránh vẽ bậy
            self.pen_up()
    ...
    def process_gcode_line_and_wait(self, line):
        """Phân tích, gửi lệnh G-code và chờ hoàn thành. SỬA ĐỔI ĐỂ NỘI SUY G0/G1."""
        ...
        for part in parts[1:]:
            ...
            try:
                val = float(part[1:])
                if code == 'X': x = val; hasX = True
                elif code == 'Y': y = val; hasY = True
                elif code == 'Z': z = val
                elif code == 'F': f = val
                elif code == 'P': p = val
            except ValueError:
                logging.warning(f"Bỏ qua G-code part không hợp lệ: '{part}'"); continue

        # --- XỬ LÝ Z, F, P (Giữ nguyên logic chờ đợi) ---
        if z is not None:
        ...
        # --- XỬ LÝ G0/G1 XY - THAY ĐỔI CHÍNH Ở ĐÂY ---
        if (cmd_code == 'G0' or cmd_code == 'G1') and hasX and hasY:
            if x is None or y is None:
                self.log(f"Lỗi G-code: Thiếu X/Y '{line}'", tag="ERROR"); return False

            # Lấy tọa độ hiện tại (hiển thị)
            current_x_disp = -self.current_position.get('x', 0.0)
            current_y = self.current_position.get('y', 30.0) # Giả sử Y không đổi nhiều

            # Tọa độ đích (hiển thị) từ G-code
            target_x_disp = x
            target_y = y

            # Thực hiện nội suy và di chuyển
            if not self._interpolate_and_move(current_x_disp, current_y, target_x_disp, target_y):
                # Nếu nội suy thất bại (lỗi IK, lỗi gửi lệnh, timeout, dừng...)
                self.log(f"(!) Nội suy G1 thất bại: {line}", tag="ERROR")
                return False # Báo lỗi để dừng G-code

            # Nội suy thành công, không cần gửi thêm lệnh nào khác cho dòng G1 này
            cmd_to_send = None
            action_py = True # Đánh dấu là đã xử lý hoàn toàn trong Python
            is_move = True # Vẫn coi đây là một lệnh di chuyển

        # --- XỬ LÝ CÁC LỆNH G/M KHÁC (Giữ nguyên logic) ---
        elif cmd_code == 'G4':
        ...
        elif cmd_code == 'G28': cmd_to_send = "h"; is_move = True
        elif cmd_code.startswith(("M3", "M03")): cmd_to_send = ('d' if not self.pen_is_down else None)
        elif cmd_code.startswith(("M5", "M05")): cmd_to_send = ('u' if self.pen_is_down else None)
        elif cmd_code == 'M17': cmd_to_send = "enable"
        elif cmd_code.startswith(("M18", "M84")): cmd_to_send = "disable"
        elif cmd_code.startswith(("G90", "G91", "G20", "G21")): logging.debug(f"Bỏ qua G-code: {line}"); action_py = True
        # Chỉ xử lý Z/F mà không có XY
        elif (cmd_code == 'G0' or cmd_code == 'G1') and (z is not None or f is not None) and not (hasX or hasY):
            action_py = True # Z/F đã được xử lý ở trên
        elif (cmd_code == 'G0' or cmd_code == 'G1') and not (hasX or hasY or z is not None or f is not None):
             action_py = True # Dòng G0/G1 trống
        else:
            # Các lệnh khác không được xử lý trong logic G0/G1 XY
            if not cmd_to_send and not action_py: # Nếu chưa có lệnh nào được xác định
                 logging.warning(f"Bỏ qua G-code không xác định hoặc không cần xử lý: {line}")
                 action_py = True # Coi như thành công

        # --- Gửi lệnh (nếu có) và chờ ---
        if cmd_to_send:
            ...
            # Đã xử lý trong Python (G4, G90, G1 XY đã nội suy, Z/F đơn lẻ...)
            return True
        else:
            # Không có lệnh cần gửi (ví dụ M3 khi bút đã hạ)
            return True
